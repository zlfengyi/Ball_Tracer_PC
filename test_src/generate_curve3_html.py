...
const D = %%DATA_JSON%%;
(function(){
const cfg = D.config, obs = D.observations, preds = D.predictions;
const s0 = preds.filter(p=>p.stage===0);
const s1 = preds.filter(p=>p.stage===1);
...
document.getElementById('st').innerHTML=[
  ...
  stat('COR', cfg.cor),
  stat('ideal_hit_z', cfg.ideal_hit_z+'mm'),
...
// ═══ Chart 0: All-in-One ═══
{
  ...
  const tr=[
    ...
    {x:s1.map(p=>p.ct-t0), y:s1.map(p=>p.x), name:'S1 X', mode:'markers',
     ...
     hovertemplate:'t=%{x:.3f}s<br>pred x=%{y:.1f} mm<extra>S1 X</extra>'},
    {x:s1.map(p=>p.ct-t0), y:s1.map(p=>p.y), name:'S1 Y', mode:'markers',
     ...
     hovertemplate:'t=%{x:.3f}s<br>pred y=%{y:.1f} mm<extra>S1 Y</extra>'},
    {x:s1.map(p=>p.ct-t0), y:s1.map(p=>p.z), name:'S1 Z', mode:'markers',
     ...
     hovertemplate:'t=%{x:.3f}s<br>pred z=%{y:.1f} mm<extra>S1 Z</extra>'},
  ...
  Plotly.newPlot('c0',tr,{
    ...
    xaxis:{title:'Time (s)',...GS}, yaxis:{title:'Value (mm)',...GS},
  },{responsive:true});
}
...
// ═══ Chart 1: X / Y / Z / Lead subplots ═══
{
  ...
  ['x','y','z'].forEach((k,i)=>{
    ...
    tr.push({x:oT,y:obs.map(o=>o[k]),name:`Ball ${k.toUpperCase()}`,mode:'markers',
      ...
      hovertemplate:`t=%{x:.3f}s<br>${k}=%{y:.1f} mm<extra>Ball ${k.toUpperCase()}</extra>`,
      yaxis:ya,xaxis:'x'});
    tr.push({x:s0.map(p=>p.ct-t0),y:s0.map(p=>p[k]),name:`S0 ${k.toUpperCase()}`,mode:'markers',
      ...
      hovertemplate:`t=%{x:.3f}s<br>pred ${k}=%{y:.1f} mm<extra>S0</extra>`,
      yaxis:ya,xaxis:'x'});
    tr.push({x:s1.map(p=>p.ct-t0),y:s1.map(p=>p[k]),name:`S1 ${k.toUpperCase()}`,mode:'markers',
      ...
      hovertemplate:`t=%{x:.3f}s<br>pred ${k}=%{y:.1f} mm<extra>S1</extra>`,
      yaxis:ya,xaxis:'x'});
  });
  // lead time
  ...
  Plotly.newPlot('c1',tr,{
    ...
    yaxis:{title:'X (mm)',...GS,domain:[0.78,1]},
    yaxis2:{title:'Y (mm)',...GS,domain:[0.53,0.75]},
    yaxis3:{title:'Z (mm)',...GS,domain:[0.28,0.50]},
...
// ═══ Chart 2: 3D ═══
{
  Plotly.newPlot('c2',[
    {x:obs.map(o=>o.x),y:obs.map(o=>o.y),z:obs.map(o=>o.z),
     ...
     hovertemplate:'t=%{text}s<br>x=%{x:.1f}<br>y=%{y:.1f}<br>z=%{z:.1f}<extra>Ball</extra>',
    ...
    {x:s0.map(p=>p.x),y:s0.map(p=>p.y),z:s0.map(p=>p.z),
     ...
     hovertemplate:'t=%{text}s<br>x=%{x:.1f}<br>y=%{y:.1f}<br>z=%{z:.1f}<extra>S0</extra>',
    ...
    {x:s1.map(p=>p.x),y:s1.map(p=>p.y),z:s1.map(p=>p.z),
     ...
     hovertemplate:'t=%{text}s<br>x=%{x:.1f}<br>y=%{y:.1f}<br>z=%{z:.1f}<extra>S1</extra>',
     text:s1.map(p=>(p.ct-t0).toFixed(3))},
  ],{
    ...DL,
    title:{text:'3D Trajectory (all throws, color=time)',font:{size:13,color:'#a0a0c0'}},
    scene:{xaxis:{title:'X(mm)',...GS,backgroundcolor:'#16213e'},
           yaxis:{title:'Y(mm)',...GS,backgroundcolor:'#16213e'},
           zaxis:{title:'Z(mm)',...GS,backgroundcolor:'#16213e'},bgcolor:'#16213e'},
  },{responsive:true});
}
})();
function sw(i){
...
def main():
    ...
    out = args.output or os.path.join(
        os.path.dirname(args.input) or ".", "curve3_interactive.html")
